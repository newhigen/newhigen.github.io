const MacScreenshot = require('./mac_screenshot');
const PRCommenter = require('./comment_with_files');

class MacScreenshotAndComment {
    constructor() {
        this.screenshot = new MacScreenshot();
    ...
    async capturePRAndComment(prNumber, message = null) {
        ...
        try {
            ...
            const screenshotResults = await this.screenshot.capturePRScreenshots(prNumber);
...
*자동 생성된 스크린샷입니다.*`;
    ...
    async captureCustomAndComment(prNumber, names, message = null) {
        console.log(`PR #${prNumber} 커스텀 스크린샷 촬영 및 댓글 작성 시작...`);

        try {
            // 1. 커스텀 스크린샷 촬영
            console.log('1단계: 커스텀 스크린샷 촬영 중...');
            const screenshotResults = await this.screenshot.captureCustomScreenshots(names);
...
if (require.main === module) {
    const screenshotAndComment = new MacScreenshotAndComment();
    ...
    if (command === 'pr' && args[1]) {
        ...
        const prNumber = args[1];
        const names = args.slice(2, -1); // 마지막 인자는 메시지
        const message = args[args.length - 1];

        screenshotAndComment.captureCustomAndComment(prNumber, names, message)
            .then(success => {
                if (!success) {
                    process.exit(1);
                }
        ...
        console.log('사용법:');
        console.log('  node mac_screenshot_and_comment.js pr <PR번호> [메시지]');
        console.log('  node mac_screenshot_and_comment.js custom <PR번호> <name1> <name2> ... [메시지]');
        console.log('');
        console.log('예시:');
        console.log('  node mac_screenshot_and_comment.js pr 57');
        console.log('  node mac_screenshot_and_comment.js pr 57 "변경사항 확인 완료"');
        console.log('  node mac_screenshot_and_comment.js custom 57 main_page about_page "커스텀 스크린샷"');
    }
}

module.exports = MacScreenshotAndComment;
