const ScreenshotMerger = require('./screenshot_merger');
const PRCommenter = require('./comment_with_screenshot');

class ScreenshotAndComment {
    constructor() {
        this.merger = new ScreenshotMerger();
    ...
    async capturePRAndComment(prNumber, message = null) {
        ...
        try {
            ...
            const screenshotPath = await this.merger.capturePRChanges(prNumber, `pr_${prNumber}`);
            ...
            console.log(`스크린샷 생성 완료: ${screenshotPath}`);
...
*자동 생성된 스크린샷입니다.*`;
    ...
    async captureCustomAndComment(prNumber, urls, selectors = [], message = null) {
        ...
        try {
            ...
            const screenshotPath = await this.merger.captureAndMerge(urls, selectors, `custom_pr_${prNumber}`);
...
if (require.main === module) {
    const screenshotAndComment = new ScreenshotAndComment();
    ...
    if (command === 'pr' && args[1]) {
        ...
        console.log('사용법:');
        console.log('  node screenshot_and_comment.js pr <PR번호> [메시지]');
        console.log('  node screenshot_and_comment.js custom <PR번호> <url1> <url2> ... [메시지]');
        console.log('');
        console.log('예시:');
        console.log('  node screenshot_and_comment.js pr 57');
        console.log('  node screenshot_and_comment.js pr 57 "변경사항 확인 완료"');
        console.log('  node screenshot_and_comment.js custom 57 https://example.com "커스텀 스크린샷"');
    }
}

module.exports = ScreenshotAndComment;
