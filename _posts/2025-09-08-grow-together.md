...
![조직](../MediaFiles/Images/%EC%A1%B0%EC%A7%81.png)
...
1. 이미 있는 지식 활용
    - 서로 연결 _ 제텔카스텐[^6] _ 과 충돌 _ 차이/비교 - 『1만 시간의 재발견』[^3]
2. 외부 재료 자기화
    - if not 내부에서 수렴 _ 좋은 모델 『1만 시간의 재발견』[^3]
3. B,C를 개선하는 프로세스 만들기
    - 회고?
4. 일찍, 자주, 작게라도 피드백 받기 ⇒ 실패에서 학습하기
5. 능력 UP하는 도구와 환경을 점진적으로. = 중력을 줄여나감. (메타포) _ 에너지 관리 『일의 감각』[^7]
    - but 완벽한 도구와 환경은 없다.
...
b1) 난이도 낮추기: 아기 버전  먼저 → 정식 버전 _ 『린 소프트웨어 개발』[^2]
...
☑ 맥락을 이해하고 개선해야 한다. _ 맥락을 고려하라 『실용주의 사고와 학습』[^1]
...
- 여러개 = 내 자신과 일치 DOWN ⇒ 불안간 DOWN
    - _ opposite to. 결론을 내리는 연습? _ 『완벽한 선택은 없다』[^8]
...
삼투압적 의사소통 - '협력' _ 이동 (1원칙 7가지 중) 『린 소프트웨어 개발』[^2]
...
- 일의 단계 X ⇒
    ...
    - 일 진행 → 추정치 ↑ _ burndown chart 『린 소프트웨어 개발』[^2]
