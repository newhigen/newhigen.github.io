...
원래도 생각이 많은 편이지만 항상 생각이 차 있는 상태를 유지하지는 않았지만[^1], 올해는 내 머릿속 해변을 밀물 썰물처럼 생각이 차들어왔다.
...
학습에 관련된 책들[^2]<sup>,</sup>[^3]에서 쓰기의 중요성을 말했었고(읽기가 입력이라면 쓰기는 출력), 그런 목적으로 글쓰기가 좋겠다 싶었다.
...
'자기화'를 해야 그 지식을 나의 것으로 만들 수 있다.[^2]<sup>,</sup>[^4]
...
출처가 기억은 나지 않지만 인간은 확고한 생각을 한다고 착각하지만 실제로 그 생각을 말해보거나 적어보라고 하면 그제서야 구체적으로 디테일들을 생각해내기 시작한다고 했다.[^5]

그래서 생각을 써 내려감으로 내가 가지고 있던 생각에 대한 막연한 인식[^5]을 탄탄한 사고로 바꾸고, 그 과정에서 내 생각과 관련된 근거들을 찾게 되고, 그러면서 내 생각에서 보완할 허점들과 근거들을 얻게 되는 것이다.
...
그래서 학습과 생각의 측면에서 논리의 전개가 타당한지, 내가 적은 의견이 누군가에게 상처가 되거나[^6]<sup>,</sup>[^7] 내가 편견을 가지고 있진 않은지를 본다.
...
인간의 의지력은 한정되어 있기 때문에[^9] 공개적 글쓰기는 내가 덜 의식하고 재고하는 장치로써 작동한다.
...
인간이기에 실수하는 것은 당연하고, 하지만 그 실수를 어떻게 관리하느냐가 중요하다는 것을.[^4]
...
<emphasis><b>나는 글쓰기를 통해 더 생각하고 효과적으로 학습하며, 공개하여 한 번 더 생각하기를</b></emphasis> 기대하고,
<emphasis><b>타인은 고민의 실마리를, 실수에서 용기를 얻기를</b></emphasis> 바란다.[^11]
